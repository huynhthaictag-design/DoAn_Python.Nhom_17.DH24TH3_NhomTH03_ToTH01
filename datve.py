...
class QuanLyDatVe:
    ...
    def taoGiaoDien(self):
        ...
        tk.Button(frame_search, text="Tìm", command=self.timTheoMaVe, bg="#4da6ff", fg="white", font=("Arial", 9)).pack(side="left", padx=(6, 0))
        ...
        tk.Button(frame_buttons, text="Thêm Vé", width=12, bg="#28a745", fg="white", command=self.themVe).pack(side=tk.LEFT, padx=10)
        tk.Button(frame_buttons, text="Sửa Vé", width=12, bg="#ffc107", fg="black", command=self.suaVe).pack(side=tk.LEFT, padx=10)
        tk.Button(frame_buttons, text="Xóa Vé", width=12, bg="#dc3545", fg="white", command=self.xoaVe).pack(side=tk.LEFT, padx=10)
        ...
        self.tree_datve.bind('<<TreeviewSelect>>', self.chonHang)
        self.tree_datve.bind('<Motion>', self.khiDiChuotVaoBang)
        self.tree_datve.bind('<Leave>', self.khiRoiChuotKhoiBang)
    ...
    def timTheoMaVe(self):
        ...
        try:
            ...
            messagebox.showerror("Lỗi Dữ Liệu", "Mã Vé phải là số nguyên."); return self.taiDuLieu()
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if not records:
                    messagebox.showwarning("Không Tìm Thấy", f"Không tìm thấy Vé có Mã: {mave_search}"); self.taiDuLieu(); return
